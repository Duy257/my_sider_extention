...
export default defineBackground(() => {
  ...
  chrome.runtime.onConnect.addListener((port) => {
    ...
    port.onMessage.addListener(async (message: AiPortRequest) => {
      ...
      try {
        ...
        const thinkingMode =
          message.thinkingMode ?? runtime.config.thinkingMode;
        const thinkingParams = getThinkingParams(
          runtime.config.providerId,
          thinkingMode,
          runtime.config.model,
        );
        const devStreamParams = getDevStreamParams(
          runtime.config.providerId,
          runtime.config.devMode,
        );
        ...
        await streamChatCompletion({
          ...
          callbacks: {
            onConnecting: () =>
              send({
                type: "AI_STREAM_CONNECTING",
                requestId: message.requestId,
              }),
            onFirstToken: () => {
              emitter?.onFirstToken();
              send({
                type: "AI_STREAM_FIRST_TOKEN",
                requestId: message.requestId,
              });
            },
            onDelta: (delta) =>
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        ...
        if (sender.tab?.id) {
          try {
            ...
            await chrome.tabs.sendMessage(sender.tab.id, {
              ...
              title: message.title,
              actionPosition: {
                top: message.position?.top ?? 200,
                left: message.position?.left ?? 200,
              },
        ...
        try {
          ...
          const readerTab = await chrome.tabs.create({
            url: chrome.runtime.getURL(
              `/reader.html?requestId=${message.requestId}`,
            ),
          });
          ...
          timeoutHandle = setTimeout(() => {
            ...
            if (trace) {
              trace = failToolTrace(
                trace,
                Date.now(),
                "Reader handoff timeout.",
              );
            }
            if (readerTab.id !== undefined) {
              chrome.tabs
                .sendMessage(readerTab.id, {
          ...
          cleanupHandoff();
          const errorMsg =
            error instanceof Error ? error.message : "Open reader failed.";
          if (trace) {
            trace = failToolTrace(trace, Date.now(), errorMsg);
          }
          sendResponse({
    ...
    if (message.type === "READER_SAVE_SESSION") {
      import("../src/core/storage/index")
        .then(({ getSavedResults, saveSavedResults }) => {
  ...
  chrome.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === "read-with-ai" && tab?.id) {
      ...
      chrome.runtime
        .sendMessage({ type: "OPEN_READING_COMPANION", requestId })
        .catch(() => {});
    }
  });
});
