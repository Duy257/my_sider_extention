...
export function SavedResults(props: {
  ...
  return (
    <section className="space-y-3 p-3">
      {props.results.length === 0 ? <p className="text-sm text-zinc-400">Chưa có kết quả nào.</p> : null}
      {props.results.map((result) => (
        <article key={result.id} className="rounded border border-zinc-800 p-3">
          ...
          <button className="mt-2 rounded bg-zinc-800 px-2 py-1 text-xs" onClick={() => props.onDelete(result.id)}>Xóa</button>
        </article>
      ))}
    </section>
  );
